import sys, importlib, pandas as pd, math
sys.path.append('.')
m = importlib.import_module('app_clean')

# Inputs matching the screenshot
row = {
    'battery_capacity_kwh': 100.0,
    'efficiency_wh_per_km': 150.0,
    'top_speed_kmh': 180.0,
    'acceleration_0_100_s': 10.0,
    'fast_charging_power_kw_dc': 150.0,
    'battery_type_Lithium-ion': 0,
    'battery_type_Lithium Iron Phosphate (LFP)': 0,
    'battery_type_NMC': 1,
}

def try_model(fname):
    import sys, importlib, pandas as pd, math
    sys.path.append('.')
    m = importlib.import_module('app_clean')

    # Inputs matching the screenshot
    row = {
        'battery_capacity_kwh': 100.0,
        'efficiency_wh_per_km': 150.0,
        'top_speed_kmh': 180.0,
        'acceleration_0_100_s': 10.0,
        'fast_charging_power_kw_dc': 150.0,
        'battery_type_Lithium-ion': 0,
        'battery_type_Lithium Iron Phosphate (LFP)': 0,
        'battery_type_NMC': 1,
    }

    def try_model(fname):
        model = m.safe_load_model(fname)
        if model is None:
            return None
        df = pd.DataFrame([row])
        means = m.compute_feature_means()
        aligned, missing = m.align_row_to_model(model, df, default_values=means)
        raw = m.predict_with_model(model, aligned)
        # extract raw numeric if possible
        raw_val = None
        if raw:
            try:
                raw_val = float(raw[0])
            except Exception:
                raw_val = None
        scalers = m.find_candidate_scalers()
        target_stats = m.compute_target_stats()
        rescaled = None
        method = None
        # try each scaler and record first plausible
        for s in scalers:
            try:
                inv = m.inverse_scale_predictions(s, raw if raw is not None else [])
                if inv:
                    v = float(inv[0])
                    # plausible if within 0..2000
                    if not math.isnan(v) and v>0 and v<2000:
                        rescaled = v
                        method = f'scaler:{type(s).__name__}'
                        break
            except Exception:
                continue
        if rescaled is None:
            # use rescale_prediction
            if raw_val is not None:
                rescaled = m.rescale_prediction(raw_val, scalers, target_stats)
                method = 'rescale_prediction'
        return {
            'file': fname,
            'raw': raw,
            'raw_val': raw_val,
            'missing_count': len(missing),
            'rescaled': rescaled,
            'method': method,
            'target_stats': target_stats,
        }

    models = m.list_model_files()
    print('Found models:', models)
    results = []
    for f in models:
        r = try_model(f)
        results.append(r)
        print('---')
        print('Model:', f)
        if r is None:
            print('  failed to load')
            continue
        print('  raw:', r['raw'])
        print('  raw_val:', r['raw_val'])
        print('  missing_count:', r['missing_count'])
        print('  method:', r['method'])
        print('  rescaled:', r['rescaled'])
        print('  target_stats:', r['target_stats'])

    # Summarize models that gave reasonable rescaled outputs
    print('\nSummary of reasonable models (rescaled > 0):')
    for r in results:
        if r and r['rescaled'] and r['rescaled']>0:
            print(f" {r['file']} -> {r['rescaled']:.1f} km via {r['method']}")
